...
if(!empty($_POST['u'])) {
	foreach ($_POST['u'] as $uid) {
		$id = filter($uid);
		mysql_query("update {$const['TBL_USERS']} set banned='1' where id='$id'");
	}
 }
...
if(!empty($_POST['u'])) {
	foreach ($_POST['u'] as $uid) {
		$id = filter($uid);
		mysql_query("update {$const['TBL_USERS']} set banned='0' where id='$id'");
	}
 }
 $ret = $_SERVER['PHP_SELF'] . '?'.$_POST['query_str'];;
...
if(!empty($_POST['u'])) {
	foreach ($_POST['u'] as $uid) {
		$id = filter($uid);
		mysql_query("delete from {$const['TBL_USERS']} where id='$id'");
	}
 }
 $ret = $_SERVER['PHP_SELF'] . '?'.$_POST['query_str'];;
...
if(!empty($_POST['u'])) {
	foreach ($_POST['u'] as $uid) {
		$id = filter($uid);
		mysql_query("update {$const['TBL_USERS']} set approved='1' where id='$id'");
		
	list($to_email) = mysql_fetch_row(mysql_query("select user_email from {$const['TBL_USERS']} where id='$uid'"));	
...
@mail($to_email, "User Activation", $message,
    "From: \"Member Registration\" <do-not-reply@occasionsonline.com>\r\n" .
...
$rs_all = mysql_query("select count(*) as total_all from {$const['TBL_USERS']}") or die(mysql_error());
$rs_active = mysql_query("select count(*) as total_active from {$const['TBL_USERS']} where approved='1'") or die(mysql_error());
$rs_total_pending = mysql_query("select count(*) as tot from {$const['TBL_USERS']} where approved='0'");						   
...
?>
    <h2><font color="#FF0000">Administration Page</font></h2>
      ...
      <table width="80%" border="0" align="center" cellpadding="10" cellspacing="0" style="background-color: #E4F8FA;padding: 2px 5px;border: 1px solid #CAE4FF;" >
        <tr>
          <td><form name="form1" method="get" action="<?php echo PAGE_ADMIN; ?>">
	  ...
	  if($get['q'] == '') { 
	  $sql = "select * from {$const['TBL_USERS']} $cond"; 
	  } 
	  else { 
	  $sql = "select * from {$const['TBL_USERS']} where `user_email` = '$_REQUEST[q]' or `user_name`='$_REQUEST[q]' ";
	  }
	  ...
	  // outputting the pages
		...
		echo "<a href=\"{constant('PAGE_ADMIN')}?$qstr&page=$page_no\">$page_no</a> ";
		$i++;
		}
		echo "</div>";
		}  ?>
		</p>
		<form name "searchform" action="<?php  echo PAGE_ADMIN; ?>" method="post">
        <table width="100%" border="0" align="center" cellpadding="2" cellspacing="0">
          ...
          <tr> 
            ...
            <td> <font size="2"><a href="javascript:void(0);" onclick='$.get("<?php  echo PAGE_DO; ?>",{ cmd: "approve", id: "<?php echo $rrows['id']; ?>" } ,function(data){ $("#approve<?php echo $rrows['id']; ?>").html(data); });'>Approve</a> 
              <a href="javascript:void(0);" onclick='$.get("<?php  echo PAGE_DO; ?>",{ cmd: "ban", id: "<?php echo $rrows['id']; ?>" } ,function(data){ $("#ban<?php echo $rrows['id']; ?>").html(data); });'>Ban</a> 
              <a href="javascript:void(0);" onclick='$.get("<?php  echo PAGE_DO; ?>",{ cmd: "unban", id: "<?php echo $rrows['id']; ?>" } ,function(data){ $("#ban<?php echo $rrows['id']; ?>").html(data); });'>Unban</a> 
...
{
$rs_dup = mysql_query("select count(*) as total from {$const['TBL_USERS']} where user_name='$post[user_name]' OR user_email='$post[user_email]'") or die(mysql_error());
...
mysql_query("INSERT INTO {$const['TBL_USERS']} (`user_name`,`user_email`,`pwd`,`approved`,`date`,`user_level`)
...
echo "<div class=\"msg\">User created with password $pwd....done.</div>"; 
      ...
      <table width="80%" border="0" cellpadding="5" cellspacing="2" class="myaccount">
        <tr>
          <td><form name="form1" method="post" action="<?php  echo PAGE_ADMIN; ?>">
              <p>User ID 
