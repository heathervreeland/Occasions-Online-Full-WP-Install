		...
		if( $total > 0 ) {
			...
			while ( $images->fetchRecord() ) {
				...
				if (file_exists(ABSPATH . $img_name)) {
					...
					$img_nodes .= "<image id='$img_num' source='$img_name' title='' description='' thumbnail='$img_name' />";
				}
			}
		}
	}
...
if ($_POST['submitted'] == "1") {
	...
	mail('ben@benvigil.com', $subject, $msg, $headers);
...
if ($a['showadd'] == '1' || $travelpolicy <> '') {
	...
	if ($a['showadd'] == '1') {
		...
		$s .= "{$a['mapaddr']}{$a['city']}, {$a['stat']} {$a['zipc']}";
	}
	$s .= $travelpolicy;
	$s .= '</p>';
}
...
if ($a['accp'] == 1) {$s .= '<img src="/media/images/logo-paypal.jpg" /> ';}
$s .= '</p>';
$s .= '</div> <!-- class="post" -->';
