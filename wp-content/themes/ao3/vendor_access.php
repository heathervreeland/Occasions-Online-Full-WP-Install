...
if (in_array($slug, array('doimage'))) {
	//page_protect();
	include "vendor_access/$slug.php";
	exit;
}
...
if (in_array($slug, array('login','logout','forgot','register','thankyou','activate'))) {
	get_header();
	ob_start();
...
if (in_array($slug, array('admin','home','profile','settings', 'leads', 'events', 'event', 'upgrade', 'upgraded', 'shows'))) {
	get_header();
	page_protect();
	set_fake_admin();
	set_profile_status();
