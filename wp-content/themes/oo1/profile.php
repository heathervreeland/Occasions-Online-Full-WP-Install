...
// ====================================================================
// FORCED MOBILE/FULL PROFILE SITE -- override
// ====================================================================
// basically, if 
//		(a) the user is ASKING for a mobile view or 
//		(b) we detect a mobile browser AND the user has not previously
//			requested a full page...
//	then flag it to show mobile.
global $browser_mobile;
if ( $cmd == 'm' || ($browser_mobile && ao_get_mobileview() != '2') ) {
	ao_set_mobileview(1);
}

// if the user is ASKING for a full page, cookie them to force it
if ($cmd == 'p') {
	ao_set_mobileview(2);
}

// ====================================================================
// SHOW MOBILE PROFILE SITE?
// ====================================================================
if (ao_get_mobileview() == '1') {
	include "profile_mobile.php";
	ao_log($a['id'], 'profile_mobileview');
	exit;
}
...
if ($_POST['submitted'] == "1") {
	...
	$to      = $a['emai'];
	$subject = 'Found you on OccasionsOnline.com ('.$txt_name.')';
...
if ($_POST['review_submitted'] == "1") {
	...
	****************************************************************/
	...
	if (empty($err)) {
		...
		mail($to, $subject, $msg, $headers, AO_EMAIL_FLAGS);
		mail(AO_ADMIN_EMAIL, $subject, $msg, $headers, AO_EMAIL_FLAGS);
		mail(AO_OFFICE_EMAIL, $subject, $msg, $headers, AO_EMAIL_FLAGS);
		mail(AO_TECH_EMAIL, $subject, $msg, $headers, AO_EMAIL_FLAGS);
