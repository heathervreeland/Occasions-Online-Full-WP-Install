...
/* Template Name: Vendor Profile 7 */
...
if ($_POST['review_submitted'] == "1") {
	...
	****************************************************************/
	...
	if (empty($err)) {
		...
		pod_query("UPDATE wp_pod_tbl_vendor_profiles SET rating = (SELECT AVG(rating) FROM wp_pod_tbl_comments WHERE vendor='" . $a['id'] . "') WHERE id = " . $a['id'] . " AND hide=0");
