...
define ("AO_TECH_EMAIL",		'"OM Webmaster" <webmaster@occasionsonline.com>');
...
$browser_iphone = iif(strpos($_SERVER['HTTP_USER_AGENT'], 'iPhone') || strpos($_SERVER['HTTP_USER_AGENT'], 'iPod'), 1, 0);
$browser_android = iif(strpos($_SERVER['HTTP_USER_AGENT'], 'Android'), 1, 0);
$browser_webos = iif(strpos($_SERVER['HTTP_USER_AGENT'], 'webOS'), 1, 0);
$browser_mobile = iif($browser_iphone || $browser_android || $browser_webos, 1, 0);
...
$ao_mobileview = $_COOKIE['AO3_MOBILEVIEW'];
...
function ao_set_mobileview($content) {
	global $ao_mobileview;
	$ao_mobileview = $content;
	setcookie('AO3_MOBILEVIEW', $content, time()+60*60*24*30, '/');
}

function ao_get_mobileview() {
	global $ao_mobileview;
	return $ao_mobileview;
}
