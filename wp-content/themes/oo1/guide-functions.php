...
function get_vendorfields(&$vpod) {
	...
	$a_profile = array(
		...
		"summary" 				=>	ao_trim_to_length(preg_replace('/\s\s+/', ' ', str_replace('&nbsp;', ' ', strip_tags(htmlspecialchars_decode($vpod->get_field('description'))))), 250),
		...
		"video_flv_last"		=>	$vpod->get_field('video_flv_last'),
	...
	if ($a_profile['ad_url']		!= '' && substr($a_profile['ad_url'], 0, 4) != "http") 		{$a_profile['ad_url'] = 'http://' . $a_profile['ad_url'];}
	...
	$a_profile['aurl'] = $a_profile['ad_url'];
...
function get_logo_block(&$a) {
	...
	if (file_exists("$img_dst_logo/$pid/logo.jpg")) {
		echo "<p class=\"vendor_desc\">The following main image has been uploaded and is already part of your profile.</p>";
		echo '<div class="vendor_imgblock">';
?>
		<div class="vendor_imgthumb" id="img_logo">
			<img src="<?php echo "$img_web_logo/$pid/logo.jpg?$random_int"; ?>" /><br />
			<div class="vendor_imgdelete" id="del_logo" onclick='$("#del_logo").html("Deleting . . .");$.get("<?php echo PAGE_DOIMAGE, '/', $pid; ?>",{cmd:"deletelogo"},function(data){if(data == "1"){$("#img_logo").remove();}else{alert("There was an error deleting the main image.");$("#del_logo").html("Delete Image");}});'>Delete Main Image</div>
		</div>
<?php
		echo '</div>';
	}
	else {
		echo "<p class=\"vendor_desc\">No main image has been uploaded yet.</p>";
	}
}

function get_video_block(&$a) {

	$pid = $a['id'];
	if ($a['video_count'] > 0) {
		echo "<p class=\"vendor_desc\">The following videos have been uploaded and are already part of your profile.</p>";
		$vid_id = 0;
		echo '<div class="vendor_vidblock">';
		foreach ($a['videolist_video'] as $thumb) {
?>
			<div class="vendor_vidthumb" id="vid_<?php echo $vid_id; ?>">
				<div class="vendor_viddelete" id="del_<?php echo $vid_id; ?>" onclick='$("#del_<?php echo $vid_id; ?>").html("Deleting . . .");$.get("<?php echo PAGE_DOIMAGE, '/', $pid; ?>",{cmd:"deletevideo",video:"<?php echo $a['videolist_names'][$vid_id]; ?>"},function(data){if(data == "1"){$("#vid_<?php echo $vid_id; ?>").remove();}else{alert("There was an error deleting the video.");$("#del_<?php echo $vid_id; ?>").html("Delete");}});'>Delete</div>
				<div class="vendor_vidname"><?php echo $a['videolist_names'][$vid_id]; ?></div>
			</div>
<?php
			$vid_id++;
		}
		...
		echo "<p class=\"vendor_desc\">No videos have been uploaded to your profile.</p>";
	}
}
